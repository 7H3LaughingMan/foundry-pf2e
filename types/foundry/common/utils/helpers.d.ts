 ...
 */
export function throttle<T extends AnyFunction>(callback: T, delay: number): T;
 ...
 * @param options.strict Throw an Error if deepClone is unable to clone something instead of
 *                       returning the original
 * @param options._d An internal depth tracker
 * @return The clone of that data
 */
export function deepClone<T>(original: T, options?: { strict?: boolean; _d?: number }): T;
 ...
 * @param options.deletionKeys Apply special logic to deletion keys. They will only be kept if the
...
export function diffObject(
    original: object,
    other: object,
    options?: { inner?: boolean; deletionKeys?: boolean; _d?: number },
): object;
 ...
 */
export function getRoute(path: string, { prefix }?: { prefix: Maybe<string> }): string;
 ...
 * @param options.insertKeys Control whether to insert new top-level objects into the resulting
 *                           structure which do not previously exist in the original object.
 * @param options.insertValues Control whether to insert new nested values into child objects in
 *                             the resulting structure which did not previously exist in the
 *                             original object.
 * @param options.overwrite Control whether to replace existing values in the source, or only
 *                          merge values which do not already exist in the original object.
 * @param options.recursive Control whether to merge inner-objects recursively (if true), or
 *                          whether to simply replace inner objects with a provided new value.
 * @param options.inplace Control whether to apply updates to the original object in-place
 *                        (if true), otherwise the original object is duplicated and the
 *                        copy is merged.
 * @param options.enforceTypes Control whether strict type checking requires that the value of a
 *                             key in the other object must match the data type in the original
 *                             data to be merged.
 * @param options.performDeletions Control whether to perform deletions on the original object if
 ...
 * @param options.base The base to use. In base 10 a kilobyte is 1000 bytes. In base 2 it is
 *                     1024 bytes.
 */
