...
export function getInMemory<T>(obj: foundry.documents.abstract.ClientDocument | foundry.canvas.placeables.Token, ...path: string[]): T | undefined {
    return foundry.utils.getProperty(obj, `modules.${module.id}.${path.join(".")}`) as T | undefined;
}

export function setInMemory<T>(obj: foundry.documents.abstract.ClientDocument | foundry.canvas.placeables.Token, ...args: [...string[], T]): boolean {
    const value = args.pop() as T;
    return foundry.utils.setProperty(obj, `modules.${MODULE.id}.${args.join(".")}`, value);
}

export function getOrSetInMemory<T>(obj: foundry.documents.abstract.ClientDocument | foundry.canvas.placeables.Token, ...args: [...string[], () => T]): T {
...
export function deleteInMemory(obj: foundry.documents.abstract.ClientDocument | foundry.canvas.placeables.Token, ...path: string[]): boolean {
...
export function isChatMacro<T extends foundry.documents.Macro>(doc: unknown): doc is T {
    return doc instanceof foundry.documents.Macro && doc.type === "chat";
}

export function isScriptMacro<T extends foundry.documents.Macro>(doc: unknown): doc is T {
    return doc instanceof foundry.documents.Macro && doc.type === "script";
}

export function isUuidOf(uuid: string, type: DocumentType | DocumentType[] | readonly DocumentType[]): uuid is DocumentUUID {
    if (!uuid) return false;

    const types = R.isArray(type) ? type : [type];
    const result = foundry.utils.parseUuid(uuid);
    return R.isNonNullish(result?.type) && R.isNonNullish(result?.documentId) && types.includes(result.type as DocumentType);
}
